...
def execute(coder, code, lang, contest, exec_args, input_file_urls, output_file_urls, input_file_hash,
    ...
    for (index, url) in enumerate(input_file_urls, start=0):
        if FILE_HASHES.get(input_file_hash[index]) is not None:
            input_testfile = FILE_HASHES.get(input_file_hash[index])
        else:
            input_testfile = urlretrieve(url, os.path.join(staticdir, input_file_urls[index].split("/")[-1]))[0]
            FILE_HASHES[input_file_hash[index]] = input_testfile

        if FILE_HASHES.get(output_file_hash[index]) is not None:
            output_testfile = FILE_HASHES.get(output_file_hash[index])
        else:
            output_testfile = urlretrieve(url, os.path.join(staticdir, output_file_urls[index].split("/")[-1]))[0]
            FILE_HASHES[output_file_hash[index]] = output_testfile
